...
if __name__ == "__main__":
    ...
    for _ in range(10):
        action = policy(torch.zeros(1,720)).detach().numpy().squeeze()
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                # obs[:1]=sin_phase
                # obs[1:2]=cos_phase
                obs=np.concatenate([cmd * cmd_scale,qj,dqj,last_action,omega,gravity_orientation])
                # obs[:3]= cmd * cmd_scale
                # obs[3:24]=qj
                # obs[24:45]=dqj
                # obs[45:66]=action
                # obs[66:69]=omega
                # obs[69:72]=gravity_orientation
                ...
                obs_copy = np.clip(obs, -100, 100)

                obs_hist = np.array(hist_obs).flatten()
                print(obs_hist, "ssssssssssssssss")

                hist_obs.append(obs_copy)
                # hist_obs.popleft()
                policy_input = np.concatenate([obs_hist, obs_copy])
                # print(policy_input,"sss")
